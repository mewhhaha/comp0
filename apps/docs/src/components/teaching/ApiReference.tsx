...
export function ApiReference({ imports, parts, className }: ApiReferenceProps) {
  ...
  return (
    <section className={cn("max-w-full min-w-0", className)}>
      ...
      <div className="-mx-4 -my-2 mt-4 overflow-x-auto sm:-mx-6 lg:-mx-10">
        <div className="inline-block min-w-full px-4 py-2 align-middle sm:px-6 lg:px-10">
          <table className="w-full min-w-md border-collapse text-left">
            <thead>
              <tr className="border-b border-zinc-950/10 dark:border-white/10">
                <th className="py-2 pr-4 text-sm/6 font-semibold whitespace-nowrap text-zinc-950 dark:text-white">
                  Part
                </th>
                <th className="py-2 pr-4 text-sm/6 font-semibold whitespace-nowrap text-zinc-950 dark:text-white">
                  Renders
                </th>
                <th className="py-2 text-sm/6 font-semibold whitespace-nowrap text-zinc-950 dark:text-white">
                  Description
                </th>
              </tr>
            </thead>
            <tbody>
              {parts.map((part) => (
                ...
                >
                  <td className="py-2.5 pr-4 align-top text-sm/6 whitespace-nowrap">
                    ...
                    {part.props && (
                      <dl className="mt-2 grid gap-1.5 border-l-2 border-zinc-950/5 pl-3 dark:border-white/10">
                        {part.props.map((partProp) => (
                          <div key={partProp.name} className="text-xs/5">
                            <dt className="inline">
                              <code className="font-mono font-medium text-teal-800 dark:text-teal-200">
                                {partProp.name}
                              </code>{" "}
