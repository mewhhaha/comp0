...
function triggerNode(entry: Numbered, rest: Numbered[], value?: Numbered): DiagramNode {
  ...
  return { type: "control", owner: entry, shape, value, chevron };
}
...
function PartName({ children, muted = false }: { children: ReactNode; muted?: boolean }) {
  return (
    <span
      className={cn(
        "font-mono text-xs/5 font-medium",
        muted ? "text-zinc-500 dark:text-zinc-400" : "text-zinc-700 dark:text-zinc-200",
      )}
    >
      {children}
    </span>
...
function Skeleton({ className }: { className?: string }) {
  return (
    <span
      aria-hidden="true"
      className={cn("h-2 rounded-full bg-zinc-300/80 dark:bg-zinc-600/80", className)}
    />
  );
}

function ControlNode({ node }: { node: Extract<DiagramNode, { type: "control" }> }) {
  const { owner, shape, value, chevron } = node;
  if (shape === "checkbox") {
    return (
      <div className="relative inline-flex items-center gap-2.5 self-start pt-1">
  ...
  if (shape === "slider") {
    return (
      <div className="relative grid max-w-64 gap-2 pt-1">
        <Pin number={owner.number} />
        <PartName>{owner.part.name}</PartName>
  ...
  if (shape === "icon") {
    return (
      <div className="relative inline-flex size-10 shrink-0 items-center justify-center self-start rounded-lg bg-zinc-200 dark:bg-zinc-700">
        <Pin number={owner.number} />
  ...
  if (shape === "link") {
    return (
      <div className="relative inline-flex items-center gap-1.5 self-start pt-1">
        <Pin number={owner.number} />
        <span className="font-mono text-xs/5 font-medium text-teal-700 underline decoration-2 underline-offset-4 dark:text-teal-300">
          {owner.part.name}
        </span>
      </div>
    );
  ...
  if (shape === "ghost") {
    return (
      <div className="relative inline-flex items-center gap-2 self-start rounded-lg border-2 border-dashed border-zinc-300 px-4 py-2 opacity-80 dark:border-zinc-600">
        <Pin number={owner.number} />
        <PartName muted>{owner.part.name}</PartName>
      </div>
    );
  }
  if (shape === "field" || shape === "search" || shape === "textarea" || shape === "stepper") {
    return (
      <div
        className={cn(
          "relative flex w-full max-w-72 gap-2 rounded-lg border border-zinc-300 bg-white px-3 dark:border-zinc-600 dark:bg-zinc-800",
  ...
  return (
    <div
      className={cn(
        "relative inline-flex items-center gap-2 self-start rounded-lg px-4 py-2",
    ...
    >
      ...
      <span
        className={cn(
          "font-mono text-xs/5 font-medium",
          shape === "button" ? "text-white dark:text-zinc-900" : "text-zinc-700 dark:text-zinc-200",
        )}
      >
        {owner.part.name}
      </span>
      {value && (
        <span className="relative ml-1 inline-flex items-center rounded bg-white/20 px-1.5 py-0.5 dark:bg-zinc-950/15">
          <Pin number={value.number} />
          <span
            className={cn(
              "font-mono text-[0.6875rem]/4",
              shape === "button" ? "text-white dark:text-zinc-900" : "text-zinc-600",
            )}
          >
            {value.part.name}
          </span>
      ...
      {chevron && (
        <ChevronDown
          className={cn(
            "size-3.5",
            shape === "button" ? "text-white/80 dark:text-zinc-900/80" : "text-zinc-500",
          )}
          aria-hidden="true"
        />
      )}
          ...
          <span key={index} className="contents">
            ...
            <span
              className={cn(
                ...
                index === 0 &&
                  "bg-teal-600/10 text-teal-800 ring-1 ring-teal-600/30 dark:bg-teal-400/10 dark:text-teal-200 dark:ring-teal-400/30",
                index !== 0 &&
                  "bg-zinc-100 ring-1 ring-zinc-950/10 dark:bg-zinc-800 dark:ring-white/10",
              )}
            >
    ...
    <div className="grid gap-1.5 pt-1">
      {[0, 1, 2].map((index) => (
        <div
          key={index}
          className={cn(
            "relative flex min-h-8 items-center gap-2.5 rounded-md px-2.5",
            index === 1 && "bg-teal-600/10 text-teal-800 dark:bg-teal-400/10 dark:text-teal-200",
          )}
        >
          {index === 0 && <Pin number={owner.number} />}
          {shape === "radio" && (
          ...
          {index === 0 ? (
            ...
            <Skeleton className={index === 1 ? "w-20" : "w-14"} />
          )}
          {shape !== "radio" && index === 1 && (
            <Check
  ...
  return (
    <>
      {nodes.map((node, index) => {
        ...
        if (node.type === "frame") {
          return (
            <div
              key={key}
              className={cn(
                "relative grid grid-cols-[minmax(0,1fr)] gap-3 rounded-xl p-4 pt-3",
            ...
            >
              ...
              <p
                className={cn(
                  "font-mono text-xs/5 font-medium",
                  node.variant === "provider" && "text-teal-700 dark:text-teal-300",
                  node.variant === "container" && "text-zinc-500 dark:text-zinc-400",
                )}
              >
                {node.owner.part.name}
                {!node.owner.part.ownsDom && (
                  <span className="ml-2 font-sans font-normal text-zinc-500 dark:text-zinc-400">
                    invisible · shares state
                  </span>
                )}
              </p>
        ...
        if (node.type === "panel") {
          return (
            <div key={key} className="grid justify-items-start">
              ...
              />
              <div className="relative grid w-full max-w-72 grid-cols-[minmax(0,1fr)] gap-2 rounded-xl bg-white p-3 pt-2.5 shadow-lg ring-1 ring-zinc-950/10 dark:bg-zinc-800 dark:shadow-none dark:ring-white/15">
                <Pin number={node.owner.number} />
                <PartName muted>{node.owner.part.name}</PartName>
                <DiagramNodes nodes={node.children} />
              </div>
            </div>
          );
        ...
        if (node.type === "label") {
          return (
            <div key={key} className="relative inline-flex items-center gap-2 self-start pt-1">
              <Pin number={node.owner.number} />
        ...
        if (node.type === "chip") {
          return (
            <div
              key={key}
              className="relative inline-flex items-center self-start rounded-md bg-zinc-100 px-2 py-1 ring-1 ring-zinc-950/10 dark:bg-zinc-700 dark:ring-white/10"
            >
              <Pin number={node.owner.number} />
              <PartName muted>{node.owner.part.name}</PartName>
            </div>
