...
export function Example() {
    ...
    const content = (
      ...
      >
        ...
        <div className="mb-4 flex items-center justify-between gap-2">
          ...
          {view === "canvas" && (
            <InventoryMoveHandle className="min-h-11 shrink-0 cursor-grab touch-none rounded border border-zinc-200 px-2 text-sm outline-teal-600 focus-visible:outline-2 dark:border-zinc-700 dark:outline-teal-400">
              Move
            </InventoryMoveHandle>
          )}
        </div>
        <div className="grid gap-4">
          {card.outputs.map((port) => (
            <ConnectOutput
              key={port.value}
              {...port}
              className="flex min-h-11 items-center justify-between gap-2 rounded border border-zinc-200 px-2 text-sm outline-teal-600 focus-visible:outline-2 data-selected:ring-2 data-selected:ring-teal-600 dark:border-zinc-700 dark:outline-teal-400"
            >
              <span>
          ...
          {card.inputs.map((port) => (
            <ConnectInput key={port.value} {...port} className="grid gap-1.5">
              <ConnectInputTrigger className="flex min-h-11 items-center gap-2 rounded border border-zinc-200 px-2 text-sm outline-teal-600 focus-visible:outline-2 data-available:ring-2 data-available:ring-teal-600 dark:border-zinc-700 dark:outline-teal-400">
                <span aria-hidden="true">○</span>
                {port.label} · {port.kind}
              </ConnectInputTrigger>
              <ConnectInputSelect className="min-h-11 w-full min-w-0 rounded border border-zinc-200 bg-white px-1 text-base outline-teal-600 focus-visible:outline-2 dark:border-zinc-700 dark:bg-zinc-900 dark:outline-teal-400" />
              <ConnectDisconnect className="min-h-11 justify-self-start rounded px-1 text-sm text-zinc-600 outline-teal-600 focus-visible:outline-2 disabled:opacity-40 dark:text-zinc-400 dark:outline-teal-400">
                Disconnect
              </ConnectDisconnect>
            </ConnectInput>
          ))}
        ...
        {view === "canvas" && (
          <InventoryResizeHandle className="mt-4 touch-none min-h-11 rounded border border-zinc-200 px-2 text-sm outline-teal-600 focus-visible:outline-2 dark:border-zinc-700 dark:outline-teal-400">
            Resize
          </InventoryResizeHandle>
        )}
      </ConnectCard>
  ...
  let board = <div className="grid gap-4 xl:grid-cols-3">{cardElements}</div>;
  if (view === "canvas")
    board = (
      <Inventory
        ...
        className="h-[80rem] list-none gap-2"
      >
        {cardElements}
      </Inventory>
    );
    ...
    <div>
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-xl text-sm/5 text-zinc-600 dark:text-zinc-400">
            Tap an output then a matching input, or choose a source from its menu. Cards fit the
            page; switch to Canvas to drag connections, move cards, or resize them. Swipe the canvas
            background to pan.
          </p>
        </div>
        <button
          type="button"
          className="min-h-11 shrink-0 rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-teal-600 focus-visible:outline-2 dark:border-zinc-700 dark:outline-teal-400"
          onClick={() => {
      ...
      <fieldset className="mb-3 flex flex-wrap gap-2" aria-label="Connection view">
        {(["cards", "canvas"] as const).map((choice) => (
          <button
            ...
            className="min-h-11 rounded-lg border border-zinc-200 px-4 text-sm outline-teal-600 aria-pressed:border-teal-600 aria-pressed:bg-teal-50 focus-visible:outline-2 dark:border-zinc-700 dark:outline-teal-400 dark:aria-pressed:bg-teal-950"
          >
            {choice === "cards" ? "Cards" : "Canvas"}
          </button>
        ))}
      </fieldset>
      <div className="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-800">
        <Connect
          ...
          className={`relative bg-zinc-50 p-3 dark:bg-zinc-950 ${view === "canvas" ? "min-w-[64rem]" : ""}`}
        >
          {view === "canvas" && <ConnectLines className="z-10 text-teal-600 dark:text-teal-400" />}
          {board}
        </Connect>
      </div>
      {view === "canvas" && (
